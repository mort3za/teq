...
`)
  ...
  function syncLangOptions() {
    ...
    const current2 = langEl.value;
    langEl.replaceChildren(new Option(t("common.allLanguages"), ""), ...opts.map((c) => new Option(langName(c), c)));
    langEl.value = opts.includes(current2) ? current2 : "";
  }
  function render() {
    const rows = filtered();
